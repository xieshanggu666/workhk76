...
- **多章远征**：创建跨章节远征（默认 3 章），击败章节首领后携带牌组、锻造成长、遗物与金币
  ...
  重复开章（400/409）与重复结算（状态机只流转一次）都被拦截。开新章时章号/总章数以
  远征归属（runs/expeditions 权威列）为准，交接快照里的旧章号不再覆盖新章——委托的
  章号/剩余期限/挂单 deadline/领奖章号因此与章节严格一致；2.4.0 前被旧章号污染的存档
  首次载入（续局/行动）即自愈，旧日志按旧语义逐位兼容回放并在版本缝对齐。
...
- `POST /api/runs/{id}/act {action,...}` 行动（choose_node / play / end_turn / claim_reward / forge / shop_buy / shop_remove / commission_accept / commission_claim）
...
- 远征章节 run 的初始状态由 `_new_run_state(seed, carry)` 构造：在线开章与回放重建共用
  ...
  归属章号以开章参数（= runs.chapter 权威列）为准，carry 快照中的旧章号仅作缺省回退，
  避免跨章 run 被上一章章号覆盖（2.4.0 修复）；回放旧版本 create 事件时按录制时的
  旧章号语义重建，并把状态的 `rules_version` 还原为录制版本（该字段参与 ckpt 哈希），
  在首个 >=2.4.0 动作处把章号对齐到权威归属，保证受影响旧日志逐位复演、零误报 mismatch。
